...
for cycles in 1 2 4 8; do
    ./run-se.sh sequential/main "cache-latency/$cycles" \
...
for size in 16kB 32kB 64kB 128kB; do
    ./run-se.sh sequential/main "cache-size/$size" \
        --l1d-size "$size" --arg 16384 --arg 40
done

# 4 MiB arrays, four passes, identical sums.
for program in sequential random; do
    ./run-se.sh "$program/main" "sequential-vs-random/$program" \
        --arg 1048576 --arg 4
done

for order in ijk ikj; do
    ./run-se.sh matmul/main "matmul/$order" --arg 96 --arg "$order"
done

for stride in 1 1025 8191; do
    ./run-se.sh dram-patterns/main "dram-patterns/$stride" \
